...
6. **Drama vs Fiction.** Plays and dramatic literature get `Drama`, NOT `Fiction`. Shakespeare gets BOTH `Drama` and `Shakespeare`. Verse plays still get `Drama`, not poetry.
...
10. **LCSH (Library of Congress Subject Headings) are authoritative for genre selection within the domain.** Map LCSH terms to the closest tag (e.g., LCSH "Stoicism" → `Stoicism` in `philosophy`; LCSH "Detective and mystery stories — United States" → `Fiction` + `[Proposed] Mystery`).
