...
11b. **Author-pattern tags are personalization signal.** When the metadata includes a "Tags frequently applied to other books by this author in the user's library" field, treat it as a strong signal that this book is in the user's existing reading patterns. They override generic LCSH-derived suggestions when they conflict, but they do NOT override LCC domain assignment (rule 1). The line includes a sample size — larger samples (≥5) indicate a strongly established pattern; smaller samples (3–4) are still meaningful but use them as a tiebreaker rather than a wholesale replacement. Apply the matching tags when the book plausibly fits them; do not apply them mechanically when the book's content clearly diverges from the pattern.
